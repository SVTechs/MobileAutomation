...
class Hg:
    def __init__(self, device, config):
        ...
        self.packageName = "com.phoenix.read"
        self.packageDir = './bin/package/hg.apk'
        self.stop_timer = None
        self.like = [922, 1729, 1040, 1891]
        self.collect = [938, 1481,1028, 1657]
        self.firstVideo = [30, 362, 360, 858]
        self.mission = ["openBox", "watchAd", "watchVideo"]#, "goShopping", "watchAd", "watchVideo"
    ...
    async def close_popups(self):
        ocr_results = await self.get_ocr_results()
        sign_popups = filter_by_text(ocr_results, ['今日签到'])
        if len(sign_popups) > 0:
            self.logger.info(f"{self.device.serial} find sign popups")
            click = filter_by_text(ocr_results, ['立即签到'])
        ...
        new_user_popups = filter_by_text(ocr_results, ['新用户必得'])
        if len(new_user_popups) > 0:
            self.logger.info(f"{self.device.serial} find new user popups")
            self.device.press_back()
            return

        add_main_popups = filter_by_text(ocr_results, ['添加到主屏幕'])
        if len(add_main_popups) > 0:
            self.logger.info(f"{self.device.serial} find add main popups")
    ...
    def on_start(self):
        ...
        self.logger.info(f"{self.device.serial} started the Hg task.")
        self.device.close_app(self.packageName)
    ...
    def on_stop(self):
        try:
            self.logger.info(f"{self.device.serial} stopped the Hg task.")
            self.device.close_app(self.packageName)
            if self.stop_timer:
    ...
    async def check_in_home(self):
        ...
        if not self.activities['main'] in activity :
            ...
            return False
        elif len(filter_by_text(ocr_results, ['推荐', '电影', '电视剧'])) >= 3:
    ...
    async def click_welfare(self):
        ...
        ocr_results = await self.get_ocr_results()
        click = filter_by_text(ocr_results, ['福利'])
        if len(click) > 0:
            self.device.click(click[0].rec)
            await self.handle_popups()
        else:
    ...
    async def open_box(self):
        ...
        ocr_results = await self.get_ocr_results()
        box_ocr = filter_by_text(ocr_results,['开宝箱'])
        if len(box_ocr) == 0:
        ...
        ocr_results = await self.get_ocr_results()
        watch_ocr = filter_by_text(ocr_results, ['看视频'])
        if len(watch_ocr) == 0:
    ...
    async def watch_video(self):
        ...
        self.device.click(self._scaled_coordinates(self.firstVideo))
        ...
        self.random_sleep(random.uniform(90, 110))
    ...
    async def watch_ad(self):
        ...
        ad_click_ocr =[]
        for i in range(3):
            ad_click_ocr = await self.cap_and_find(['立即领取'], [], True)
    ...
    async def close_ad(self):
        ...
        if self.activities['ad'] in activity:
            ...
            while i < 25 and not self.device.should_stop:
                ...
                close = filter_by_text(ocr_results, ['关闭'])
                if len(get_award) > 0:
                ...
                #     break
                    ...
                    self.logger.info(f"{self.device.serial}: get award close")
                    self.device.click(close[0].rec)
                    break
                else:
                    self.random_sleep(3)
                    i += 1
        ...
        elif self.activities['scrollad'] in activity:
            self.logger.info(f"{self.device.serial} in scroll ad")
            ...
            self.logger.info(f"{self.device.serial}: unknown ad activity {activity}")
            self.current_task_completed = True
    ...
    async def watch_more(self):
        ...
        if len(click_get_more) > 0:
            ...
            agree_click = await self.cap_and_find(["领取奖励"])
            ...
            one_more_click = filter_by_text(ocr_results,["领取奖励"])
            if len(one_more_click) > 0:
                ...
                reject_click = filter_by_text(ocr_results,["放弃奖励","坚持退出"])
                if len(reject_click) > 0:
                    self.device.click(reject_click[0].rec)
