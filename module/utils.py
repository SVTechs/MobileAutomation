...
def random_normal_distribution_int(a, b, n=3):
    """Generate a normal distributio    n int within the interval. Use the average value of several random numbers to
...
def filter_by_text(ocr_results, text_list, exclude_list=None):
    if exclude_list is None:
        exclude_list = []
    filtered_results = []
    for result in ocr_results:
        if any(text_item in result.text for text_item in text_list) and not any(
                exclude_item in result.text for exclude_item in exclude_list):
            filtered_results.append(result)
    return filtered_results


def within_percentage(percentage):
    if not (0 <= percentage <= 100):
        raise ValueError("percentage should in 0 to 100")
