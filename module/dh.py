...
class Dh:
    def __init__(self, device, config):
        ...
        self.packageName = "com.eggflower.read"
        self.packageDir = './bin/package/dh.apk'
    ...
    async def close_popups(self):
        ...
        bind_popups = filter_by_text(ocr_results, ['抖音快捷绑定'])
        if len(bind_popups) > 0:
            self.logger.info(f"{self.device.serial} find bind popups")
            self.device.press_back()
            return
    ...
    def on_start(self):
        ...
        #     return
        # self.logger.info(f"{self.device.serial} Installing Dh from {self.packageDir}...")
        # self.device.install_app(self.packageDir)
        # self.logger.info(f"{self.device.serial} installed Dh.")

        self.logger.info(f"{self.device.serial} started the Dh task.")
        self.device.close_app(self.packageName)
    ...
    async def check_in_home(self):
        ...
        if not self.activities['main'] in activity:
            ...
            return False
        elif len(filter_by_text(ocr_results, ['听书', '推荐', '短句'])) >= 3:
    ...
    async def click_home(self):
        ...
        ocr_results = await self.get_ocr_results()
        click = filter_by_text(ocr_results, ['书城'])
        if len(click) > 0:
            self.device.click(click[0].rec)
            await self.handle_popups()
        else:
    ...
    async def run(self):
        try:
            while not self.device.should_stop:
                ...
                self.logger.info(
                    f"{self.device.serial} Source Width: {self.device.width}, Height: {self.device.height}")
                self.logger.info(
                    f"{self.device.serial} Scale factors - Width: {self.scale_width}, Height: {self.scale_height}")
    ...
    async def sign_in(self):
        ...
        # swipe 5 times to find sign
        for i in range(5):
            sign_in_ocr = await self.cap_and_find(['去签到'])
            ...
            if len(sign_tomorrow_ocr) > 0:
                ...
                self.random_sleep(2)
                get_sign_ocr = await self.cap_and_find(['立即签到'])
                if len(get_sign_ocr) > 0:
                    self.logger.info(f'{self.device.serial} sign at {get_sign_ocr[0].rec}')
                    self.device.click(get_sign_ocr[0].rec)
                    self.random_sleep(2)
                get_more_ocr = await self.cap_and_find(['看视频'])
